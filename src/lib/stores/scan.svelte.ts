...
class ScanStore {
	...
	scannerStatuses = $state<Record<string, 'pending' | 'scanning' | 'completed' | 'failed'>>({});
	...
	byRisk = $derived.by(() => {
		const groups: Record<string, ScanResult[]> = { Zero: [], Low: [], Medium: [], High: [] };
		for (const item of this.items) {
	...
	handleProgress(progress: ScanProgress) {
		switch (progress.type) {
			...
			case 'ScannerStarted':
				...
				this.scannerStatuses = { ...this.scannerStatuses, [progress.category]: 'scanning' };
				break;
			case 'ScannerCompleted':
				this.completedScanners++;
				this.scannerStatuses = { ...this.scannerStatuses, [progress.category]: 'completed' };
				break;
			case 'ScannerFailed':
				this.completedScanners++;
				this.scannerStatuses = { ...this.scannerStatuses, [progress.category]: 'failed' };
				break;
			case 'Cancelled':
