...
class ActivityStore {
	...
	filteredEntries = $derived.by(() => {
		...
		if (this.filter === 'commands') result = result.filter((e) => e.level === 'Command');
		else if (this.filter === 'errors') result = result.filter((e) => e.level === 'Error' || e.level === 'Warning');
		else if (this.filter === 'success') result = result.filter((e) => e.level === 'Success');
	...
	latestEntry = $derived(this.entries.length > 0 ? this.entries[this.entries.length - 1] : null);

	errorCount = $derived(this.entries.filter((e) => e.level === 'Error' || e.level === 'Warning').length);
